...
public class CollectionCache implements BrokerPoolService {
    ...
    public Collection getOrCreate(final XmldbURI collectionUri, final Function<XmldbURI, Collection> creator) {
        return cache.get(key(collectionUri), uri -> unwrapLocked(creator.apply(XmldbURI.create(uri))));
    }
    ...
    public void put(final Collection collection) {
        cache.put(key(collection.getURI()), unwrapLocked(collection));
    }
    ...
    /**
     * We must not store LockedCollections in the CollectionCache!
     *
     * @param collection The Collection input to cache
     * @return the actual Collection object to cache
     */
    private Collection unwrapLocked(final Collection collection) {
        ...
        if(collection instanceof LockedCollection) {
            ...
            return ((LockedCollection)collection).getCollection();
        } else {
            return collection;
        }
    }
