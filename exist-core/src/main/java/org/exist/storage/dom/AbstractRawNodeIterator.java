 ...
 * document.
 */
public abstract class AbstractRawNodeIterator implements IRawNodeIterator {

    private final static Logger LOG = LogManager.getLogger(AbstractRawNodeIterator.class);

    protected DBBroker broker;
    protected final DOMFile db;
     ...
     * The concrete superclass is responsible for
     * positioning the iterator before the specified
     * start node.
     *
     ...
     */
    protected AbstractRawNodeIterator(final DBBroker broker, final DOMFile db) {
        this.broker = broker;
        this.db = db;
    }

    @Override
    public void seek(final NodeHandle node) throws IOException {
        RecordPos rec = null;
